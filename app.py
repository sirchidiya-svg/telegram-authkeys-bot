...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        welcome_message, parse_mode="MarkdownV2", reply_markup=reply_markup
    )
...
async def save_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message.reply_to_message:
        ...
        if generated_key is None:
            await update.message.reply_text(
                "⚠️ That message isn't a key this bot generated, so I can't attach it.\n"
                "Reply directly to a /numeric, /alphanumeric, or regenerated key message — "
                "or run /save without replying to save just your title and details."
            )
            return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "regenerate_numeric":
        ...
        await context.bot.send_message(
            chat_id=query.message.chat_id, text=full_commands_text()
        )
